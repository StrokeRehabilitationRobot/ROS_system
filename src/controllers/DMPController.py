...
class DMPController(object):
    ...
    def update_dmp_file(self,file,start,goal):
        ...
        self.runner_z = dmp.Mod_DMP_runner(self.file + "_z.xml", start[2], goal[2])
        self.last_dmp = [(),(),()]
        self.err = [0,0,0]

    def step(self, state, dt, tau=1, current_state=[0,0,0], force=[0,0,0]):
        """
        ...
        :return:
        """
        err_x = abs(current_state[1] - self.last_x[0])
        err_y = abs(current_state[2] - self.last_y[0])
        err_z = abs(current_state[0] - self.last_z[0])
        ...
        self.last_x = (x_t, xd_t, xdd_t)
        self.last_y = (y_t, yd_t, ydd_t)
        self.last_z = (z_t, zd_t, zdd_t)
