...
#     for i in xrange(3):
#         q[i ] = round(src.tools.helper.encoder_to_angle(upstream[i * 3 + 0 + 1]), 2)
#         qd[i] = round(src.tools.helper.encoder_to_angle(upstream[i * 3 + 2 + 1]), 2)
#         tau[i] = src.tools.helper.filter_tau(upstream[i * 3 + 2 + 1], i)
#
#     q[2] -= 0.5 * math.pi
#
#     state.position = q
...
def udp_server():
    ...
    # udp = src.communication.UDP(9876)
    rospy.spin()
